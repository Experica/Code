...
# recordSession = "007"  #AF2
# testId = ["003","005", "006","002"]
...
plotfitstas=(u,m;dir=nothing)->begin
    ...
    imagesize = size(ulsta)[1]
    ppd = datasetFinal["ppd"]
    stisize = datasetFinal["stisize"]
    maskradius = datasetFinal["maskradius"]
    truestimsz = stisize*maskradius*2
    ucex = map(i->i.ex,datasetFinal["ulcex"][u])
    ...
    heatmap!(p[2,c],x,y,fs[c],aspect_ratio=:equal,frame=:grid,color=:coolwarm,clims=(-clim,clim),
    xlims=xylims,ylims=xylims,xticks=xylims,yticks=[],xlabel=datasetFinal["color"][c],title="Fit_$(m)"),1:cn)
