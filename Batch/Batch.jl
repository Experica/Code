...
function batchtests(tests::DataFrame,param::Dict{Any,Any}=Dict{Any,Any}();as::AbstractString="",log::Dict{Any,AbstractLogger}=Dict{Any,AbstractLogger}(),plot::Bool=true)
    ...
    for t in eachrow(tests)
        ...
        try
            ...
            elseif ID in ["ISICycle2Color","ISICycleOri"] && t.sourceformat=="Imager"
                process_cycle_imager(t.files,param;uuid=t.UUID,log,plot)
            elseif ID in ["ISIEpochOri8","ISIEpochOri12","ISIEpoch2Color","ISIEpochFlash2Color"] && t.sourceformat=="Imager"
                process_epoch_imager(t.files,param;uuid=t.UUID,log,plot)
            else
                @warn "No corresonding process function for test ID = `$(ID)`, skip `$(t.files)` from `$(t.sourceformat)` ..."
            end
        catch exc
            display("============================================")
